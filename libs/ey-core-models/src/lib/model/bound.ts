...
import type { DisruptionStatus } from './disruption-status';
import type { FlightItem } from './flight-item';
import type { ViewMetrics } from './view-metrics';
...
export interface Bound {
  /**
   * Identifier of the bound. Once air offer is stored in the cart, bound identifiers  abstract from the shopping identifier (offerNdcId).
   */
  airBoundId?: string;
  /**
   * This field is not populated in OrderAirItem.Bound
   */
  ...
  /**
   * Departure date without timezone (local time). - dateTime format: ISO 8601 see http://www.w3.org/TR/NOTE-datetime. This field will appear only in case of complex itinerary to differentiate the bounds.
   */
  departureDate?: string;
  /**
   * Type of the bound whether it is outbound or inbound. This is only populated for round trip booking.
   */
  boundType?: BoundBoundType;
  /**
   ...
   */
  ranking?: number;
  flightViewMetrics?: ViewMetrics;
}
export enum BoundBoundType {
  Outbound = 'OUTBOUND',
  Inbound = 'INBOUND',
}
