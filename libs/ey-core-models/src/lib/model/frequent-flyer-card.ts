 ...
 */
import type { CardUsage } from './card-usage';
...
export interface FrequentFlyerCard {
   ...
   */
  airlineCustomerValue?: string;
  /**
   * Reference to the air-offer biased with the Frequent Flyer.  When this link is present, the Frequent Flyer card cannot be deleted separately from the offer. This link is created once the related air-offer is added to the cart  and cannot be updated in a PATCH process. Available for Miles & Cash only.
   */
  airOfferId?: string;
  cardUsage?: CardUsage;
}
