 ...
 */
import type { ConjunctiveTravelDocument } from './conjunctive-travel-document';
import type { DxApiPublicPaymentTransaction } from './dx-api-public-payment-transaction';
import type { EventLog } from './event-log';
import type { FareInfoTravelDocument } from './fare-info-travel-document';
import type { OriginalTravelDocument } from './original-travel-document';
import type { TravelDocumentPrice } from './travel-document-price';
...
export interface TravelDocument {
  ...
  travelerTIds?: string[];
  /**
   * Insurance ids the element is associated with
   */
  insuranceIds?: string[];
  /**
   * Hotel ids the element is associated with
   */
  hotelIds?: string[];
  /**
  ...
  id: string;
  /**
   * It specifies if the document is the record generated when confirming flights (documentType =  eticket), or is used for for travel related services, such as excess baggage charges, miscellaneous charges, penalty fees, residual value, lounge access charges, etc.. (documentType =  service), or for third party contents (documentType = insurance or hotel)
   */
  documentType?: TravelDocumentDocumentType;
  /**
   * Free-flow text of max 150 characters for the endorsement corresponding to travel document.
   */
  endorsement?: string;
  price?: TravelDocumentPrice;
  paymentTransactions?: DxApiPublicPaymentTransaction[];
  fareInfos?: FareInfoTravelDocument[];
  originalDocuments?: OriginalTravelDocument[];
  /**
   * Provides the Conjunctive travel documents associated to the curent travel document
   */
  conjunctiveTravelDocuments?: ConjunctiveTravelDocument[];
  creation?: EventLog;
  /**
   * Boolean used to differentiate Ticket/EMD where coupons and segments/services are auto synchronized, with the ones not trusted.(No default value as read from the PNR).
   */
  areCouponsAndSegmentsInSync?: boolean;
  /**
   * Status of the travel document contained in the fare element at PNR level
   */
  status?: TravelDocumentStatus;
}
export enum TravelDocumentDocumentType {
  Eticket = 'eticket',
  Services = 'services',
  Insurance = 'insurance',
  Hotel = 'hotel',
