 ...
 */
import type { AirBoundItemService } from './air-bound-item-service';
import type { AirBoundsAiroffer } from './air-bounds-airoffer';
import type { AirBoundsCheapestAiroffer } from './air-bounds-cheapest-airoffer';
import type { AirPricingRecords } from './air-pricing-records';
import type { AvailabilityInfoItem } from './availability-info-item';
import type { FareInfo } from './fare-info';
import type { ShoppingProfileRanking } from './shopping-profile-ranking';
import type { Status } from './status';
import type { UpsellBenefits } from './upsell-benefits';

/**
 * The air bound will consist fare family and price information for a given bound offer.
 */
export interface AirBoundExchangeItem {
  /**
  ...
  /**
   * Tag highlighting the airBound leading to the cheapest airOffer (result of the combination of 2 airBounds)
   */
  isCheapestOffer?: boolean;
  availabilityDetails?: AvailabilityInfoItem[];
  fareInfos?: FareInfo[];
  prices?: AirPricingRecords;
  /**
   * A list of ancillary services available for this AirBound item
   */
  services?: AirBoundItemService[];
  cheapestAirOffer?: AirBoundsCheapestAiroffer;
  airOffer?: AirBoundsAiroffer;
  status?: Status;
  /**
   * FareConditionsCode provides a reference to the purchase conditions defined in the dictionary. In case of more than one bound, fare conditions might change between airbound responses.
   */
  fareConditionsCodes?: string[];
  /**
   * Indicates if the bound is composed of flights with different cabin.
   */
  isMixedCabin?: boolean;
  upsell?: UpsellBenefits;
  shoppingProfileRankings?: ShoppingProfileRanking[];
  /**
   * Indicates if the bound is composed of flights where a waiver has been applied and penalties have been waived (Dynamic Waiver product).
   */
  isWaived?: boolean;
}
