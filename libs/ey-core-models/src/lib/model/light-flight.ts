...
export interface LightFlight {
  ...
  /**
   * Flag indicating if the flight has already been flown (the default value is false).
   */
  isFlown?: boolean;
  ...
  bookingClass?: string;
  /**
   * Operational status. Possible values are: HK (confirmed), HL (waitlist),  TK (schedule change), HX (have cancelled). Flights with other values will not be returned.
   */
  statusCode?: string;
}
