 ...
 */
import type { FlightItemOperating } from './flight-item-operating';
...
export interface FlightItem {
   ...
   */
  id: string;
  cabin?: FlightItemCabin;
  /**
   * Booking class associated to the cabin
   */
  bookingClass?: string;
  operating?: FlightItemOperating;
  /**
   * Operational status. HK = confirmed, HL = waitlist, TK = schedule change confirmed, TL = schedule change waitlist, UN = unable to confirm not operating, UC = unable to confirm, HX = have cancelled, NO = no action taken. At shopping time, the only status that can be returned is HL=waitlist
   */
  statusCode?: string;
  /**
  ...
  isFlown?: boolean;
  /**
   * commercial fare family stored in the air offer,this field is populated when calling get Order or order reprice  .
   */
  commercialFareFamilyCode?: string;
  /**
   * It provides a reference to the amenities defined in the dictionary.
   */
  amenityCodes?: string[];
}
export enum FlightItemCabin {
  Eco = 'eco',
