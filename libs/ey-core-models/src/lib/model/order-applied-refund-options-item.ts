...
import type { OrderPaidAmount } from './order-paid-amount';
import type { OrderRefundAmounts } from './order-refund-amounts';
import type { OrderRefundNotificationData } from './order-refund-notification-data';
import type { OrderUsedAmount } from './order-used-amount';
import type { PenaltyDetail } from './penalty-detail';
import type { RefundPaymentTransaction } from './refund-payment-transaction';

/**
 * Detail of the refund option for an air travel document (aka flight e-ticket).  It contains the flight e-ticket number, its refund status, the amounts that will be refunded and the refund penalty if applicable.   The status at flight e-ticket level can have the following values: - refunded - pendingManualRefund - notRefundable  Note: Only e-tickets with status refundable returns refund amounts and penalty.
 */
export interface OrderAppliedRefundOptionsItem {
  /**
   * Travel Document number (e.g. 220-1638074788)
   */
  travelDocumentId: string;
  /**
   * It specifies if the document is the record generated when confirming flights (documentType =  eticket), or is used for for travel related services, such as excess baggage charges, miscellaneous charges, penalty fees, residual value, lounge access charges, etc.. (documentType =  service), or is used for third party contents, such as hotel
   */
  documentType: OrderAppliedRefundOptionsItemDocumentType;
  /**
  ...
  serviceIds?: string[];
  /**
   * Seat ids the element is associated with
   */
  seatIds?: string[];
  /**
   * Describes the status of the refund option.  For flights, the refund status can have the following possible values: - refunded - pendingManualRefund - notRefundable  For hotels, the status is either refundable or notRefundable.
   */
  status: OrderAppliedRefundOptionsItemStatus;
  /**
   * Specifies if the refund option provided to the api user is an involuntary refund. An involuntary refund can be proposed by the airline if the order has been disrupted and the airline has contracted the product.
   */
  refundType?: OrderAppliedRefundOptionsItemRefundType;
  refundAmounts?: OrderRefundAmounts;
  paidAmount?: OrderPaidAmount;
  usedAmount?: OrderUsedAmount;
  penalty?: Amount;
  penaltyDetail?: PenaltyDetail;
  /**
   * DocumentId representing the penalty paid in cash associated to a refunded ticket in Miles (exist only for ticket paid in Miles after refund)
   */
  penaltyDocmentId?: string;
  /**
   * Details on the refunded forms of payment. Returned sub-structures are \'paymentMethod\', \'amount\'.
   */
  paymentDetails?: RefundPaymentTransaction[];
  serviceRefundFee?: Amount;
  /**
   * Details about the different refund notification(s) processed during the cancellation.
   */
  refundNotifications?: OrderRefundNotificationData[];
  /**
   * Waiver code applicable for the given travel-document. All amounts mentioned above are  already considering this waiver.
   */
  waiverCode?: string;
}
export enum OrderAppliedRefundOptionsItemDocumentType {
  Eticket = 'eticket',
  Services = 'services',
  Hotel = 'hotel',
}
export enum OrderAppliedRefundOptionsItemStatus {
  Refunded = 'refunded',
  PendingManualRefund = 'pendingManualRefund',
  NotRefundable = 'notRefundable',
  Refundable = 'refundable',
}
export enum OrderAppliedRefundOptionsItemRefundType {
