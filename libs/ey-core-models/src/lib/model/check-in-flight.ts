 ...
 */
import type { FlightEndPoint } from './flight-end-point';
import type { FlightPerformanceIndicator } from './flight-performance-indicator';
import type { FlightStop } from './flight-stop';
import type { Meal } from './meal';

/**
 * Flight information when within the Departure Control System
 */
export interface CheckInFlight {
  /**
  ...
  /**
   * IATA airline code (http://www.iata.org/publications/Pages/code-search.aspx). Defines the actual aircraft owner airline operating the flight
   */
  aircraftOwnerAirlineCode?: string;
  /**
   * Aircraft owner name if the IATA airline code is not available
   */
  aircraftOwnerAirlineName?: string;
  /**
   * Suffix of the flight. IATA standard practices rule that only one flight with a given flight number can operate on a given day.  However, some issues can happen if a flight originally scheduled to depart before midnight is delayed,  and its new schedule departure date changes to the next day. In this case, to avoid two flight numbers  departing on the same date, a suffix is typically added to the delayed flight number. For example flight 6X 233 scheduled to depart on the 11th May at 23:30 may become 6X 233A scheduled to  depart on the 12th May at 00:30, in order to avoid conflicts with 6X 233 scheduled to depart on the 12th May at 23:30.
   */
  suffix?: string;
  ...
  duration?: number;
  /**
   * Duration of connecting time expressed in seconds
   */
  ...
  /**
   * Flag indicating if the flight is informational. An informational flight is booked and fully managed on another system and is only  kept in the Amadeus order for information purpose.
   */
  ...
  /**
   * Flag indicating if the flight must be approved by the government (DEI-201)
   */
  isSubjectToGovernmentApproval?: boolean;
  performanceIndicator?: FlightPerformanceIndicator;
  /**
  ...
  meals?: Meal;
  /**
   * This field is FOR FLIGHT SEARCH (SHOPPING) STEP ONLY. Provides the list of meals per booking class in the flight. B=BREAKFAST, K=CONTINENTAL BREAKFAST, L=LUNCH, D=DINNER, S=SNACK OR BRUNCH, O=COLD MEAL, H=HOT MEAL, M=MEAL (NON-SPECIFIC), R=REFRESHMENT, C=ALCOHOLIC BEVERAGES COMPLIMENTARY,F=FOOD FOR PURCHASE, P=ALCOHOLIC BEVERAGES FOR PURCHASE,  Y=DUTY FREE SALES AVAILABLE, N=NO MEAL SERVICE, V=REFRESHMENTS FOR PURCHASE, G=FOOD AND BEVERAGES FOR PURCHASE. Booking Class can be Y=Economy.
   */
  availableMealsPerBookingClass?: Meal[];
  /**
   * status of the flight
   */
  flightStatus?: CheckInFlightFlightStatus;
  /**
   ...
   */
  acceptanceStatus: CheckInFlightAcceptanceStatus;
  /**
  ...
  isPassive?: boolean;
  /**
   * Indicate if the flight requires the passenger to confirm their participation to Pilgrimage (Islamic pilgrimage to Mecca).
   */
  isPilgrimConfirmationRequired?: boolean;
  /**
   * Reference to the bound a flight belongs to. Used only for return journeys.
   */
  boundId?: string;
}
export enum CheckInFlightFlightStatus {
  Scheduled = 'scheduled',
  Departed = 'departed',
  Cancelled = 'cancelled',
  Delayed = 'delayed',
...
export enum CheckInFlightStatus {
  Scheduled = 'scheduled',
  Gated = 'gated',
  Departed = 'departed',
  Finalized = 'finalized',
...
export enum CheckInFlightAcceptanceStatus {
  NotOpened = 'notOpened',
  Opened = 'opened',
  Gated = 'gated',
  Closed = 'closed',
