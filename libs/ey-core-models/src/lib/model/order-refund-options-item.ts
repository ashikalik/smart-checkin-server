...
import type { OrderNonRefundableAmount } from './order-non-refundable-amount';
import type { OrderPaidAmount } from './order-paid-amount';
import type { OrderRefundAmounts } from './order-refund-amounts';
import type { OrderRefundMethodOption } from './order-refund-method-option';
import type { OrderUsedAmount } from './order-used-amount';
import type { PenaltyDetail } from './penalty-detail';

/**
 * Detail of the refund eligibility for each travel document (i.e. flight ticket). It contains the e-ticket number, the refund status at e-ticket level,  the amounts that will be refunded and the refund penalty if applicable.  The status at e-ticket level can have the following values: - refundable - notRefundable - manuallyRefundable: the tickets cannot be refunded by the API and needs to be refunded offline by an agent(or airline robotic application). This can happen when the  e-ticket is not eligible for automatic refund or when there was an error during the eligibility call. - unavailable: this status is returned either when there is an error during the computation of the eligibility.
 */
export interface OrderRefundOptionsItem {
  /**
   * Travel Document number (e.g. 220-1638074788)
   */
  travelDocumentId: string;
  /**
   * It specifies if the document is the record generated when confirming flights (documentType =  e-ticket), or is used for for travel related services, such as excess baggage charges, miscellaneous charges, penalty fees, residual value, lounge access charges, etc.. (documentType =  service) Could be also used for hotel refund (documentType =  hotel)
   */
  documentType: OrderRefundOptionsItemDocumentType;
  /**
  ...
  seatIds?: string[];
  /**
   * Describes the refund status of the travel document. Three possible values: refundable, notRefundable and unavailable.
   */
  status: OrderRefundOptionsItemStatus;
  /**
