...
/**
 * This structure aims at receiving 3DS data.
 */
export interface Payment3DSAuthentication {
  /**
   * Booking type, can have 3 different values: \"Online\" value needs to be sent for online channels   For instance: website is considered as Online. \"Offline\" value needs to be sent for offline channels   For instance: ARD is considered as offline. \"Exempted\" value needs to be sent when transaction is to exempted for PSD2.
   */
  deviceChannel?: Payment3DSAuthenticationDeviceChannel;
  /**
   * 3DS Transaction Identifier Mandatory when 3DS V1 data need to be sent to external provider to process payment
   */
  xid?: string;
  /**
   * CAVV - Cardholder Authentication Verification Value (VISA) When 3DS V1/V2 data need to be sent to external provider to process payment:  Either authentificationValueAVV or authentificationValueCAVV need to be sent in input.
   */
  cavv?: string;
  /**
   * AVV - Accountholder Authentification Value (MasterCard) When 3DS V1/V2 data need to be sent to external provider to process payment:  Either authentificationValueAVV or authentificationValueCAVV need to be sent in input.
   */
  avv?: string;
  /**
   * Authentication Response status (pAResStatus): Y: Authentication successful N: Authentication failed A: Attempts processing performed U: Authentication could not be performed
   */
  paresStatus?: Payment3DSAuthenticationParesStatus;
  /**
   * Verify Enrollment Response status (vEResStatus) Y: Cardholder is enrolled to use 3D Secure N: Cardholder is not enrolled to use 3D Secure U: Unable to Authenticate
   */
  veresStatus?: Payment3DSAuthenticationVeresStatus;
  /**
   * Electronic Commerce Indicator (eci) Mandatory when 3DS V1/V2 data need to be sent to external provider to process payment
   */
  eci?: string;
  /**
   * Transaction Status   Y: Payer authenticated   N: Payer not authenticated   U: Authentication could not be performed by 3DS server   A: Authentication attempted (not authenticated but proof of attempted authentication provided)   C: Challenge required (using CReq/CRes)   D: Challenge required - Decoupled authentication confirmed   R: Authentication rejected   I: Informational - requestor challenge preference acknowledged
   */
  transStatus?: Payment3DSAuthenticationTransStatus;
  /**
   * Version of the 3DS
   */
  version?: string;
...
export enum Payment3DSAuthenticationDeviceChannel {
  Online = 'ONLINE',
  Offline = 'OFFLINE',
  Exempted = 'EXEMPTED',
}
export enum Payment3DSAuthenticationParesStatus {
  Y = 'Y',
  N = 'N',
  A = 'A',
  U = 'U',
}
export enum Payment3DSAuthenticationVeresStatus {
  Y = 'Y',
  N = 'N',
  U = 'U',
}
export enum Payment3DSAuthenticationTransStatus {
  Y = 'Y',
  N = 'N',
  U = 'U',
  A = 'A',
