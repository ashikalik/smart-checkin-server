...
import type { PriceBaseDetails } from './price-base-details';
import type { PricingConditions } from './pricing-conditions';
import type { Surcharge } from './surcharge';
import type { Tax } from './tax';
import type { TravelDocumentPriceBreakdown } from './travel-document-price-breakdown';

/**
 * Price breakdowns of the travel document expressed as exhaustive amounts.    - The \'total\' price represents the price paid in this travel document.     - In case of prime booking, it includes base fare, taxes, surcharges.      - In case of exchange it contains the difference to be paid on top of the original document.     - (It may be 0 in case of exchange with no additional collection, in this case the \'totalTaxes\' is also 0).   - All the prices are usually expressed in the currency \'currencyCode\'     - The base amount can be either the amount filled or the equivalent fare amount in case the amount was filled in the different currency     - Exception: The \'base\' price, in case of exchange with no additional collection, represents the \'base\' price filed, so it may have its own currency, provided in field \'baseCurrencyCode\'
 */
export interface TravelDocumentPrice {
  /**
   * Base price. (Does not include taxes nor fees) The base amount can be either the amount filled or the equivalent fare amount in case the amount was filled in the different currency. In case of exchange with no additional collection, it represents the \'base\' price filed, so it may have its own currency, provided in field \'baseCurrencyCode\'.
   */
  base?: number;
  baseDetails?: PriceBaseDetails;
  /**
   * ISO currency code (http://www.iso.org/iso/home/standards/currency_codes.htm) Currency of the \'base\' price in exchange flow. If not specified the \'currencyCode\' is used for \'base\' price as well.
   */
  baseCurrencyCode?: string;
  /**
   * The Total price represents the price paid in this travel document.   - In case of prime booking, it includes base fare, taxes, surcharges.   - In case of exchange it contains the difference to be paid on top of the original document.   - (It may be 0 in case of exchange with no additional collection, in this case the \'totalTaxes\' is also 0).
   */
  total: number;
  /**
   * ISO currency [code](http://www.iso.org/iso/home/standards/currency_codes.htm). It allows accessing the currency dictionary, containing any information related to the currency: in particular `decimalPlaces`  can be used to determine the amount actual value and display. Example: the numerical amount `1955` associated to the `EUR` currency code (`decimalPlaces: 2`) corresponds to: `19,55 Euro`. For points (or miles) the code associated is `MIL`, for compensation/upgrade credits `ECR`. All the prices are usually expressed in the currency \'currencyCode\' - Exception: The \'base\' price, in case of exchange with no additional collection, represents the \'base\' price filed, so it may have its own currency, provided in field \'baseCurrencyCode\'
   */
  currencyCode: string;
  /**
   * Taxes breakdown
  ...
  taxes?: Tax[];
  /**
   * Sum of all taxes (may be 0 in case of exchange when the total to be paid is 0)
   */
  totalTaxes: number;
  /**
  ...
  fees?: Fee[];
  /**
   * Sum of all fees (max amount is not included)
   */
  totalFees?: number;
  discount?: Discount;
  conditions?: PricingConditions;
  /**
   * One or more splits of the current price per a given dimension (per fare family, per passenger, ...)
   */
  priceBreakdowns?: TravelDocumentPriceBreakdown[];
}
