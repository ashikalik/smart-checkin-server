 ...
 */
import type { EntitlementDocument } from './entitlement-document';
import type { Name } from './name';
import type { RegulatoryApisDetail } from './regulatory-apis-detail';
...
export interface JourneyTraveler {
   ...
   */
  accompanyingTravelerTid?: string;
  /**
   * Gender of the traveler (value \'unknown\' to be used for undisclosed gender). This attribute is available for the Cart resource. In the context of an Order, the information could be retrieved as well from regulatory documents (when specified).
   */
  gender?: JourneyTravelerGender;
  /**
   ...
   */
  regulatoryDetails?: RegulatoryApisDetail[];
  /**
  ...
  staffNumber?: string;
  /**
   * staff category of the traveler.
   */
  staffCategory?: JourneyTravelerStaffCategory;
  /**
   * List of entitlement documents associated to the traveler. These information are available only when using the operations associated to the cart `entitlement-documents` sub resource
   */
  entitlementDocuments?: EntitlementDocument[];
  /**
   * Indicates the purpose of visit for the traveler. Supported values are: - hajj: Hajj is an annual Islamic pilgrimage to Mecca. - umrah: Umrah is an Islamic pilgrimage to Mecca that can be undertaken at any time of the year.
   */
  purposeOfVisit?: JourneyTravelerPurposeOfVisit;
  /**
   * Indicates if the passenger has provided their Purpose of visit(Pilgrimage)confirmation.
   */
  isPilgrimConfirmationProvided?: boolean;
  /**
   * Indicates if the traveler is willing to travel separately from the other travelers they are linked to in the PNR if all the travelers can\'t be accomodated on the flight they are on standby.
   */
  isVoluntaryToSplitTravelers?: boolean;
}
export enum JourneyTravelerGender {
  Male = 'male',
  Female = 'female',
  Unspecified = 'unspecified',
  Unknown = 'unknown',
}
export enum JourneyTravelerStaffCategory {
  Bookable = 'bookable',
  Standby = 'standby',
}
export enum JourneyTravelerPurposeOfVisit {
  Hajj = 'hajj',
  Umrah = 'umrah',
}
