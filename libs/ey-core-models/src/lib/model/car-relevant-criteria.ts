 ...
 */
export interface CarRelevantCriteria {
  /**
  ...
  providers?: Provider[];
  /**
   * List of car supplier company names. A company name here is the brand name of car supplier on a broker. For example, Rentalcars is the broker, while Avis is the company name of Avis supplier.
   */
  companyNames?: string[];
  /**
   * List of car passenger capacities. For example, [1,2,3,4,5] for all the car capacities from 1 to 5 passengers. Or [4,9,16] for the car capacity of 4, 9 or 16 passengers.
   */
  ...
  acrissCodes?: string[];
  /**
   * Sorting order operator related to a car recommendation that is used for displaying the list of car offers. priceAsc, to order car offers by price ascending; priceDesc, to order car offers by price descending; providerPreferredOrderAsc, to order car offers by car provider preference.
   */
  orderBy?: CarRelevantCriteriaOrderBy;
}
export enum CarRelevantCriteriaOrderBy {
  PriceAsc = 'priceAsc',
  PriceDesc = 'priceDesc',
  ProviderPreferredOrderAsc = 'providerPreferredOrderAsc',
}
