 ...
 */
import type { NameRegulatory } from './name-regulatory';

/**
 * Object used to manipulate (add, read, update) regulatory details (APIS data) for a passenger.
 */
export interface PassengerRegulatoryApisDocument {
  /**
  ...
  issuanceDate?: string;
  /**
   * Date after which the document is not valid anymore (date in ISO 8601, e.g. 2002-07-25: YYYY-MM-DD) Mandatory for documentType \'visa\'
   */
  expiryDate?: string;
  /**
  ...
  /**
   * Document issuance country (2-letters) code using ISO 3166-1 alpha 2 & 3 format Mandatory for documentType \'visa\', \'knownTravellerNumber\' and \'redressNumber\'
   */
  issuanceCountryCode?: string;
  name?: NameRegulatory;
  /**
   * Code of the nationality appearing on the document expressed using ISO 3166-1 alpha 2 & 3 format
   */
  nationalityCode?: string;
  /**
   * Gender of the passenger. The value \'unspecified\' must be used for non-binary gender. The value \'unknown\' must be used for undisclosed gender.
   */
  gender?: PassengerRegulatoryApisDocumentGender;
  /**
   ...
   */
  documentType: PassengerRegulatoryApisDocumentDocumentType;
  /**
   * A list of travelers associated to this generic service. If not specified, the generic service applies to all travelers
   */
  travelerIds: string[];
}
export enum PassengerRegulatoryApisDocumentGender {
  Male = 'male',
  Female = 'female',
  Unspecified = 'unspecified',
  Unknown = 'unknown',
}
export enum PassengerRegulatoryApisDocumentDocumentType {
  Passport = 'passport',
