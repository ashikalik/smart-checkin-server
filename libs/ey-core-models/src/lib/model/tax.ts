...
export interface Tax {
  ...
  code?: string;
  /**
   * In the context of a reissue, flag indicating if the tax is related to the first issue (old) or the reissue (new or refundable).
   */
  category?: TaxCategory;
  /**
   * The type of tax (airport tax, PFC tax, etc.). A PFC tax is a Passenger Facility Charge (US airport fee; see https://en.wikipedia.org/wiki/Passenger_facility_charge). Currently set only for refundable PFC taxes; for all other taxes the type is NOT set.
   */
  type?: TaxType;
  /**
  ...
  percentage?: number;
  /**
   * Specifies level / scope where the Tax is applicable (full itinerary, portion of itinerary, geographic zone etc.). Currently supports only itinerary level tax.
   */
  applicationLevel?: TaxApplicationLevel;
}
