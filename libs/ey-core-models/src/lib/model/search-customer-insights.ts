...
import type { RegulatoryApisDocument } from './regulatory-apis-document';
import type { SearchFrequentFlyer } from './search-frequent-flyer';

/**
 * Object used to define customers insights input for a search
 */
export interface SearchCustomerInsights {
  /**
   * traveler type (http://api.dev.amadeus.net/api/Development/common/codelist/PNR_PassengerType.htm)
   */
  passengerTypeCode: string;
  /**
   * Individual first name.   If person has middle name both first and middle name should be added here (e.g. Arthur Ignatius) When creating/updating regulatory details, only first name can be added here.(e.g. Arthur) When using nameType=universal, only ASCII characters are accepted ([a-zA-Z ]+) for both lastName and firstName The number of characters allowed for first name in regulatory details may differ upon configuration.
   */
  firstName?: string;
  /**
   * Individual last name.  When using nameType=universal, only ASCII characters are accepted ([a-zA-Z ]+) for both lastName and firstName The number of characters allowed for last name in regulatory details may differ upon configuration.
   */
  lastName?: string;
  /**
   * Individual title
  ...
  dateOfBirth?: string;
  /**
   * Gender of the individual (value \'unknown\' to be used for undisclosed gender).
   */
  gender?: SearchCustomerInsightsGender;
  /**
   * Individual\'s nationalities expressed as two letter country codes
   */
  nationalityCountryCodes?: string[];
  identityDocuments?: RegulatoryApisDocument[];
  contacts?: Contact[];
  frequentFlyerCards?: SearchFrequentFlyer[];
  /**
   * Temporary id that can be used to associate requested and returned travelers. It is only valid the time of the transaction.
   */
  tid: string;
}
export enum SearchCustomerInsightsGender {
  Male = 'male',
  Female = 'female',
