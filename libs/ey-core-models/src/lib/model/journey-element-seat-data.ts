...
import type { SeatmapCoordinates } from './seatmap-coordinates';
import type { SeatmapSeatProperties } from './seatmap-seat-properties';
import type { SeatmapTravelerSeatInfo } from './seatmap-traveler-seat-info';

export interface JourneyElementSeatData {
  /**
  ...
  /**
   * Check if an infant is seating on this seat, whether it is an infant associated to a traveler (PTC INF) or an infant alone (PTC INS). If the system detects there is no infant, value is false. If information is unknown, the node is not present (isInfantOnSeat not returned or null)
   */
  isInfantOnSeat?: boolean;
  /**
   * This field indicates if the seat is occupied by an infant alone (PTC INS).  If information is unknown, the node is not present (isInfantAloneOnSeat not returned or null)
   */
   ...
   */
  seatAvailabilityStatus?: JourneyElementSeatDataSeatAvailabilityStatus;
  /**
   * List of reason for restriction of seat. When calling the seatmap some seats can be unavailable for the user to be selected. Most of the time a seat is already taken, but some other time the seat selection is restricted by the airline.  For example, a seat can be available to be selected only if the user is a frequent flyer. Example on possible values:   - TARG: Target zone   - MASK: Unavailable seat   - CHSP: Chargeable seat   - ASRE: Chargeable seat   - SPTR: Chargeable seat   - PROT: Protected seat   - BLCS: Promotes seats reserved for the passenger\'s airline   - GRRS: Group   - ZONL: Legal suitability rule   - ZONM: Mandatory suitabulity rule   - THEO: Theoretical   - RSTG: Reseating   - EXST: Extra seat   - CBBG: Cabin beggage   - INFA: Infant seat   - BLCS: Codeshare seat   - ACCR: Quota Chargeable   - RDEL: Delivered   - SMCU: Booking Class   - SFRC: Reaccomodation   - ASE1: Adjacency   - AFA1: Adjacency   - ASP1: Adjacency   - THR1: Through seat   - ZONR: Releasable suitability rule   - COUR: Courtesy   - SPRQ: Request seat
   */
  reasonForRestrictionCodes?: string[];
  /**
   * Traveler\'s seat information and price
   */
  travelers?: SeatmapTravelerSeatInfo[];
  /**
   * Please refer to block section, with number of oxygen seats
   */
  blockId?: string;
  /**
   * Provides details about the nature of the seat equipment, based on the airline filing (i.e. physical seatmap)
   */
  seatType?: string;
  coordinates?: SeatmapCoordinates;
  associatedOfferItem?: Relationship;
  /**
   ...
   */
  isExitSuitabilityRequired: boolean;
  /**
   * Id of the JourneyElement the seat is associated to.
   */
  journeyElementId: string;
  previousSeat?: SeatmapSeatProperties;
  leg?: Leg;
}
export enum JourneyElementSeatDataSeatAvailabilityStatus {
  Available = 'available',
  Blocked = 'blocked',
  Occupied = 'occupied',
}
