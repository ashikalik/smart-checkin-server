 ...
 */
import type { AdditionalPaymentIndicators } from './additional-payment-indicators';
import type { Amount } from './amount';
import type { DxApiPublicPaymentMethod } from './dx-api-public-payment-method';
import type { PayerIdentification } from './payer-identification';
import type { PaymentAuthorizationInput } from './payment-authorization-input';
import type { PaymentThirdPartyItems } from './payment-third-party-items';

/**
 * A payment request contains: - the payment method to use for the payment - the amount and currency to be paid using that method - the items to be covered by this payment request The items to be covered by the payment request is optional. Several options are available for the API user: - The API user can provide a specific payment request for seat, services, and penalties (misc Ancillaries), by providing their ids. - If this is not provided, the payment request will be used to cover any items in the order Note that the payment of a specific seat and/or services should cover its full amount i.e. payment of 1 single service with multiple services is not supported. Supported payment methods are: MilesPayment, PaymentCard, ExternalPayment or AlternativePaymentMethod.
 */
export interface PaymentWithAssociationRequest {
  paymentMethod: DxApiPublicPaymentMethod;
  amount?: Amount;
  /**
  ...
  /**
   * Request this card to be presented at the airport counter.
   */
  requiredAtAirport?: boolean;
  additionalPaymentIndicators?: AdditionalPaymentIndicators;
  /**
   * A list of Misc ancillaries to be paid
   */
  miscAncillaryItemIds?: string[];
  /**
  ...
  seatItemIds?: string[];
  /**
   * A list of Insurances associated to this payment record.
   */
  insuranceIds?: string[];
  /**
   * A list of Hotels associated to this payment record.
   */
  hotelIds?: string[];
  /**
   * ID of the currency conversion Proposal to be used for the payment
   */
  currencyConversionProposalIds?: string[];
  thirdPartyItems?: PaymentThirdPartyItems;
