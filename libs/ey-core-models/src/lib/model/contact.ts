...
export interface Contact {
  ...
  travelerIds?: string[];
  /**
   * List of travelers temporary ids associated to stored in the cart. It can be used when both the contact and its associated traveler(s) are added simultaneously. Traveler association is required when the contactType has a \'notification\' purpose.
   */
  travelerTIds?: string[];
  /**
  ...
  name?: Name;
  /**
   * if set, this flag indicates that the contact belongs to an other person than the passenger (e.g. friend or family member not part of the trip).  This option is only available for mobile phone and email and for a notification purpose.
   */
  isThirdParty?: boolean;
  /**
   * To see the structure of the different types of contacts, please check [Contacts documentation](../../display/contacts.html#/ContactType/get_). Please note that \"FreeFlow\" can be returned in Digital Commerce reply but cannot be used as input.
   */
  contactType: ContactContactType;
  /**
   * List of consents given for the following contact. A consent can be granted automatically via the Digital Commerce\'s configuration  or through offline channel(s): this structure cannot be used to specify (in input) a consent when adding the contact  in the Cart or the Order.
   */
  dataConsentUsage?: DataUsageConsent[];
}
