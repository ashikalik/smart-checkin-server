 ...
 */
import type { CapturedDataByDevice } from './captured-data-by-device';

/**
 * Operation to be used for building the list of journeys available for check-in corresponding to a confirmed booking. The capturedDataByDevice is a special case, it is a data captured by a device and must be provided alone In case both capturedDataByDevice and any other data in the Journey Request is provided, Bad Request Error  will be thrown.
 */
export interface JourneyRequest {
   ...
   */
  documentType?: JourneyRequestDocumentType;
  /**
   * Numeric identifier for the order
   */
  numericOrderId?: string;
  capturedDataByDevice?: CapturedDataByDevice;
}
export enum JourneyRequestDocumentType {
  Passport = 'passport',
