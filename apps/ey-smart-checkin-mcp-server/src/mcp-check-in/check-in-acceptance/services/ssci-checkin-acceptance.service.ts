// apps/ey-smart-checkin-mcp-server/src/mcp-check-in/check-in-acceptance/services/ssci-checkin-acceptance.service.ts
...
export type CheckinAcceptanceApiResponse = {
  data?: {
    isAccepted?: boolean;
    isEligibleForVoluntaryDeniedBoarding?: boolean;
    isPartial?: boolean;
...
@Injectable()
export class CheckinAcceptanceService {
  // real endpoint base
  private readonly baseUrl =
    process.env.SSCI_CHECKIN_ACCEPTANCE_BASE_URL ??
  ...
  // cached mock
  private mockData?: CheckinAcceptanceApiResponse;
  private mockLoading?: Promise<CheckinAcceptanceApiResponse>;
   ...
   */
  async checkInAcceptance(params: {
    url?: string;
    resourceId?: string;
   ...
   */
  async getMock(): Promise<CheckinAcceptanceApiResponse> {
    return this.loadMockData();
  }

  private async loadMockData(): Promise<CheckinAcceptanceApiResponse> {
    if (this.mockData) return this.mockData;
    if (this.mockLoading) return this.mockLoading;

    const port = process.env.PORT ?? '3000';
    const url =
      process.env.CHECKIN_ACCEPTANCE_MOCK_DATA_URL ??
      `http://localhost:${port}/mocks/oneway-onepax/checkin-acceptance`;

    this.mockLoading = firstValueFrom(this.http.get<CheckinAcceptanceApiResponse>(url))
      .then((res) => {
  ...
  /**
   * REAL: POST to SSCI check-in acceptance endpoint.
   * Note: if your upstream expects a body, pass it via rawBody.
   */
  private async callUpstream(params: {
    ...
    const response$ =
      params.rawBody === undefined
        ? this.http.get<CheckinAcceptanceApiResponse>(url, {
            headers: mergedHeaders,
            timeout: 55_000,
          })
        : this.http.post<CheckinAcceptanceApiResponse>(url, params.rawBody, {
            headers: mergedHeaders,
            timeout: 55_000,
          });
