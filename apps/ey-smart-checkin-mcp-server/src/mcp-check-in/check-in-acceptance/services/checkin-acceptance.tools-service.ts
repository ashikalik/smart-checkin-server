// apps/ey-smart-checkin-mcp-server/src/mcp-check-in/check-in-acceptance/services/checkin-acceptance.tools-service.ts
...
import { CheckinAcceptanceService } from './ssci-checkin-acceptance.service';
import { ssciCheckinAcceptanceMcpTool } from '../tools/checkin-acceptance.tools';

type McpSession = { server: McpServer; transport: StreamableHTTPServerTransport };

@Injectable()
export class CheckinAcceptanceToolsService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(CheckinAcceptanceToolsService.name);
  private readonly sessions = new Map<string, McpSession>();

  constructor(private readonly acceptanceSvc: CheckinAcceptanceService) {}

  async onModuleInit(): Promise<void> {
    this.logger.log(
      'MCP check-in-acceptance started. Endpoint: /mcp-check-in/v1/checkin-acceptance',
    );
  }
  ...
  private createSession(): McpSession {
    ...
    server.registerTool(
      ssciCheckinAcceptanceMcpTool.name,
      ssciCheckinAcceptanceMcpTool.definition,
      ssciCheckinAcceptanceMcpTool.handler(this.acceptanceSvc),
    );
