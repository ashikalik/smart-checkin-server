...
import { BoardingPassService } from './ssci-boarding-pass.service';
import { ssciBoardingPassMcpTool } from '../tools/boarding-pass.tools';

type McpSession = { server: McpServer; transport: StreamableHTTPServerTransport };

@Injectable()
export class BoardingPassToolsService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(BoardingPassToolsService.name);
  private readonly sessions = new Map<string, McpSession>();

  constructor(private readonly boardingPassSvc: BoardingPassService) {}

  async onModuleInit(): Promise<void> {
    this.logger.log('MCP boarding-pass started. Endpoint: /mcp-check-in/v1/boarding-pass');
  }
  ...
  private createSession(): McpSession {
    ...
    server.registerTool(
      ssciBoardingPassMcpTool.name,
      ssciBoardingPassMcpTool.definition,
      ssciBoardingPassMcpTool.handler(this.boardingPassSvc),
    );
