...
@Controller('auto-check-in')
export class AutoCheckInOrchestratorController {
  constructor(
    private readonly orchestrator: AutoCheckInOrchestratorService,
  ...
  private attachExtrasIfEnabled(
    ...
    const enableSteps = this.configService.get<string>('IDENTIFICATION_ENABLE_STEPS') === 'true';
    const enableGoal = this.configService.get<string>('IDENTIFICATION_ENABLE_GOAL') === 'true';
