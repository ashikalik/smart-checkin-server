...
import { AncillaryCatalogueAgentController } from './ancillary-catalogue-agent.controller';
...
@Module({
  imports: [
    ...
    AiAgentModule.forFeatureAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        mcpServers: resolveAncillaryCatalogueMcpServers(configService),
  ...
  controllers: [AncillaryCatalogueAgentController],
  providers: [AncillaryCatalogueAgentService],
  exports: [AncillaryCatalogueAgentService],
})
export class AncillaryCatalogueAgentModule {}
...
const resolveAncillaryCatalogueMcpServers = (configService: ConfigService) => {
  const list = configService.get<string>('ANCILLARY_CATALOGUE_MCP_SERVER_URLS');
  if (list) {
    try {
      ...
      const servers = parsed
        ...
        .map((item, index) => ({
          ...
          name: item.name ?? `ancillary-catalogue-mcp-${index + 1}`,
      ...
      const servers = list
        ...
        .map((url, index) => ({
          url,
          name: `ancillary-catalogue-mcp-${index + 1}`,
        }));
      if (servers.length > 0) {
        return servers;
      }
  ...
  const single = configService.get<string>('ANCILLARY_CATALOGUE_MCP_SERVER_URL');
  if (single) {
    return [{ url: single, name: 'mcp-ancillary-catalogue' }];
  }
