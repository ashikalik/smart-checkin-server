...
@Controller('begin-conversation')
export class BeginConversationAgentController {
  constructor(
    private readonly orchestrator: BeginConversationAgentService,
  ...
  private attachExtrasIfEnabled(
    ...
    const enableSteps = this.configService.get<string>('BEGIN_CONVERSATION_ENABLE_STEPS') === 'true';
    const enableGoal = this.configService.get<string>('BEGIN_CONVERSATION_ENABLE_GOAL') === 'true';
