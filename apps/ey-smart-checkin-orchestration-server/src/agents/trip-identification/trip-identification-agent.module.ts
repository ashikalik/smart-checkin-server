  ...
  imports: [
    ...
    AiAgentModule.forFeatureAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        mcpServers: resolveTripIdentificationMcpServers(configService),
...
const resolveTripIdentificationMcpServers = (configService: ConfigService) => {
  const list = configService.get<string>('TRIP_IDENTIFICATION_MCP_SERVER_URLS');
  if (list) {
    try {
      ...
      const servers = parsed
        ...
        .map((item, index) => ({
          ...
          name: item.name ?? `trip-identification-mcp-${index + 1}`,
      ...
      const servers = list
        ...
        .map((url, index) => ({
          url,
          name: `trip-identification-mcp-${index + 1}`,
        }));
      if (servers.length > 0) {
        return servers;
      }
