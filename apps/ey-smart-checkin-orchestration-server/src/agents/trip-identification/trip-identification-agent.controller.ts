...
@Controller('trip-identification')
export class TripIdentificationAgentController {
  constructor(
    private readonly orchestrator: TripIdentificationAgentService,
  ...
  private attachExtrasIfEnabled(
    ...
    const enableSteps = this.configService.get<string>('TRIP_IDENTIFICATION_ENABLE_STEPS') === 'true';
    const enableGoal = this.configService.get<string>('TRIP_IDENTIFICATION_ENABLE_GOAL') === 'true';
