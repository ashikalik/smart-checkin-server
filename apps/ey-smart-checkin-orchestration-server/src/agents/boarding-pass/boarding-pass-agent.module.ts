...
@Module({
  imports: [
    ...
    AiAgentModule.forFeatureAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        mcpServers: resolveBoardingPassMcpServers(configService),
  ...
  controllers: [BoardingPassAgentController],
  providers: [BoardingPassAgentService],
  exports: [BoardingPassAgentService],
})
export class BoardingPassAgentModule {}
...
const resolveBoardingPassMcpServers = (configService: ConfigService) => {
  const list = configService.get<string>('BOARDING_PASS_MCP_SERVER_URLS');
  if (list) {
    try {
      ...
      const servers = parsed
        ...
        .map((item, index) => ({
          ...
          name: item.name ?? `boarding-pass-mcp-${index + 1}`,
      ...
      const servers = list
        ...
        .map((url, index) => ({
          url,
          name: `boarding-pass-mcp-${index + 1}`,
        }));
      if (servers.length > 0) {
        return servers;
      }
  ...
  const single = configService.get<string>('BOARDING_PASS_MCP_SERVER_URL');
  if (single) {
    return [{ url: single, name: 'mcp-boarding-pass' }];
  }
