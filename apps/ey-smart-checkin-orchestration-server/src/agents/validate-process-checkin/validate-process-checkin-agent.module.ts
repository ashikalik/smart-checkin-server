...
@Module({
  ...
  controllers: [ValidateProcessCheckInAgentController],
  providers: [ValidateProcessCheckInAgentService],
  exports: [ValidateProcessCheckInAgentService],
})
export class ValidateProcessCheckInAgentModule {}
...
const resolveIdentificationMcpServers = (configService: ConfigService) => {
  const list = configService.get<string>('VALIDATE_PROCESS_CHECKIN_MCP_SERVER_URLS');
  if (list) {
    try {
  ...
  const single = configService.get<string>('VALIDATE_PROCESS_CHECKIN_MCP_SERVER_URL');
  if (single) {
