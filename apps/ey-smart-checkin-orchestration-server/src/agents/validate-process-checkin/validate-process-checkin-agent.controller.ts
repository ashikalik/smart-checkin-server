...
@Controller('validate-process-checkin')
export class ValidateProcessCheckInAgentController {
  constructor(
    private readonly Agent: ValidateProcessCheckInAgentService,
  ...
  private attachExtrasIfEnabled(
    ...
    const enableSteps = this.configService.get<string>('VALIDATE_PROCESS_CHECKIN_ENABLE_STEPS') === 'true';
    const enableGoal = this.configService.get<string>('VALIDATE_PROCESS_CHECKIN_ENABLE_GOAL') === 'true';
