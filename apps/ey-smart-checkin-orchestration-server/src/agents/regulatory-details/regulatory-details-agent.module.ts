...
import { RegulatoryDetailsAgentController } from './regulatory-details-agent.controller';
import { RegulatoryDetailsAgentService } from './regulatory-details-agent.service';
...
@Module({
  imports: [
    ...
    AiAgentModule.forFeatureAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        mcpServers: resolveRegulatoryDetailsMcpServers(configService),
  ...
  controllers: [RegulatoryDetailsAgentController],
  providers: [RegulatoryDetailsAgentService],
  exports: [RegulatoryDetailsAgentService],
})
export class RegulatoryDetailsAgentModule {}
...
const resolveRegulatoryDetailsMcpServers = (configService: ConfigService) => {
  const list = configService.get<string>('REGULATORY_DETAILS_MCP_SERVER_URLS');
  if (list) {
    try {
      ...
      const servers = parsed
        ...
        .map((item, index) => ({
          ...
          name: item.name ?? `regulatory-details-mcp-${index + 1}`,
      ...
      const servers = list
        ...
        .map((url, index) => ({
          url,
          name: `regulatory-details-mcp-${index + 1}`,
        }));
      if (servers.length > 0) {
        return servers;
      }
  ...
  const single = configService.get<string>('REGULATORY_DETAILS_MCP_SERVER_URL');
  if (single) {
    return [{ url: single, name: 'mcp-regulatory-details' }];
  }
