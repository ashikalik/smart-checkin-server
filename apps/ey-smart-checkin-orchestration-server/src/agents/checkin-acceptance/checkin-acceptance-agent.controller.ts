...
@Controller('checkin-acceptance')
export class CheckinAcceptanceAgentController {
  constructor(
    private readonly Agent: CheckinAcceptanceAgentService,
  ...
  private attachExtrasIfEnabled(
    ...
    const enableSteps = this.configService.get<string>('CHECKIN_ACCEPTANCE_ENABLE_STEPS') === 'true';
    const enableGoal = this.configService.get<string>('CHECKIN_ACCEPTANCE_ENABLE_GOAL') === 'true';
