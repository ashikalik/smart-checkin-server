...
@Injectable()
export class CheckinAcceptanceAgentService {
  constructor(
  ...
  runAgentLoop(goal: string): Promise<{ goal: string; steps: AiAgentStep[]; final: unknown }> {
    return this.agent.runAgentLoop(goal, {
      enforceToolUse: true,
      toolChoice: 'auto',
      allowedTools: ['ssci_checkin_acceptance'],
      maxToolEnforcementRetries: this.parseNumber(
        this.configService.get<string>('CHECKIN_ACCEPTANCE_ORCHESTRATOR_TOOL_ENFORCE_RETRIES'),
      ) ?? 3,
      maxInvalidToolArgs: this.parseNumber(
        this.configService.get<string>('CHECKIN_ACCEPTANCE_ORCHESTRATOR_MAX_INVALID_TOOL_ARGS'),
      ) ?? 5,
      toolUsePrompt: this.buildToolUsePrompt(),
      systemPrompt: this.buildSystemPrompt(),
      continuePrompt:
        this.configService.get<string>('CHECKIN_ACCEPTANCE_ORCHESTRATOR_CONTINUE_PROMPT') ??
        'Continue. Use tools if needed.',
      computedNotesTemplate: this.buildComputedNotesTemplate(),
      maxModelCalls:
        this.parseNumber(this.configService.get<string>('CHECKIN_ACCEPTANCE_ORCHESTRATOR_MAX_CALLS')) ?? 6,
    });
  }
  ...
  async handleStage(
    ...
    if (record) {
      const isAccepted = record.isAccepted === true;
      const hasError = Boolean(record.error);
      if (isAccepted) {
        record.status = STAGE_STATUS.USER_INPUT_REQUIRED;
        record.continue = false;
        record.userMessage = 'Check in is successfully completed. Do you want to generate the boarding pass?';
      } else if (hasError) {
    ...
    return this.stateHelper.toStageResponse(sessionId, CheckInState.CHECKIN_ACCEPTANCE, payload, result.steps);
  }
  ...
  private buildSystemPrompt(): string {
    return this.getRequiredEnv('CHECKIN_ACCEPTANCE_ORCHESTRATOR_SYSTEM_PROMPT');
  }

  private buildToolUsePrompt(): string {
    return this.getRequiredEnv('CHECKIN_ACCEPTANCE_ORCHESTRATOR_TOOL_USE_PROMPT');
  }

  private buildComputedNotesTemplate(): string {
    return this.getRequiredEnv('CHECKIN_ACCEPTANCE_ORCHESTRATOR_COMPUTED_NOTES_TEMPLATE');
  }
