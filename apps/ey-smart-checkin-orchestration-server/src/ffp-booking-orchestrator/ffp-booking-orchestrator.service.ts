...
@Injectable()
export class FfpBookingOrchestratorService {
  constructor(
  ...
  runAgentLoop(goal: string): Promise<{ goal: string; steps: AiAgentStep[]; final: unknown }> {
    return this.agent.runAgentLoop(goal, {
      ...
      maxInvalidToolArgs: this.parseNumber(
        this.configService.get<string>('FFP_BOOKING_MAX_INVALID_TOOL_ARGS'),
      ) ?? 5,
      toolUsePrompt: this.buildToolUsePrompt(),
      systemPrompt: this.buildSystemPrompt(),
      continuePrompt:
        this.configService.get<string>('FFP_BOOKING_CONTINUE_PROMPT') ??
        'Continue. Use tools if needed.',
      computedNotesTemplate: this.buildComputedNotesTemplate(),
      maxModelCalls: this.parseNumber(this.configService.get<string>('FFP_BOOKING_MAX_CALLS')) ?? 6,
    });
  }
  ...
  private buildSystemPrompt(): string {
    return this.getRequiredEnv('FFP_BOOKING_SYSTEM_PROMPT');
  }

  private buildToolUsePrompt(): string {
    return this.getRequiredEnv('FFP_BOOKING_TOOL_USE_PROMPT');
  }

  private buildComputedNotesTemplate(): string {
    return this.getRequiredEnv('FFP_BOOKING_COMPUTED_NOTES_TEMPLATE');
  }
