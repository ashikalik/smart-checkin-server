...
import { AutoCheckInOrchestratorController } from './auto-checkin-orchestrator.controller';
import { AutoCheckInOrchestratorService } from './auto-checkin-orchestrator.service';
...
@Module({
  imports: [
    ...
    AiAgentModule.forFeatureAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        mcpServers: resolveAutoCheckInMcpServers(configService),
  ...
  controllers: [AutoCheckInOrchestratorController],
  providers: [AutoCheckInOrchestratorService],
})
export class AutoCheckInOrchestratorModule {}
...
const resolveAutoCheckInMcpServers = (configService: ConfigService) => {
    ...
    try {
      ...
      const servers = parsed
        ...
        .map((item, index) => ({
          ...
          name: item.name ?? `auto-check-in-mcp-${index + 1}`,
